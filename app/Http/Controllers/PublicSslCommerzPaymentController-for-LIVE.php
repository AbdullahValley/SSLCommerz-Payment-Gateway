...
// START Customized Business Logic.
         ...
         DB::table('orders')
            ->Insert(
                ['name' => $request->full_name,
                 'email' => $request->email,
                 'phone' => $request->phone,
                 'grand_total' => $request->session()->get('cart')->totalPrice,
        ...
        DB::table('orders')
            ...
            ->update(['order_id' => $last_transaction_id]);

// END of Customized Business Logic.


        $post_data = array();
        $post_data['total_amount'] = $request->session()->get('cart')->totalPrice; # You cant not pay less than 10
        $post_data['currency'] = "BDT";
        $post_data['tran_id'] = $last_transaction_id; // tran_id must be unique
